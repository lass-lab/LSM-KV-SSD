...
extern enum CMD_TYPE commandType[1024];
extern XTime st_read_log[AVAILABLE_OUNTSTANDING_REQ_COUNT];
extern XTime ed_read_log[AVAILABLE_OUNTSTANDING_REQ_COUNT];
extern struct STAT_PER_TYPE stat[LAST_CMD];
...
void EvictDataBufEntryForMemoryCopy(unsigned int dataBufEntry)
{
...
void DataReadFromNand(unsigned int originReqSlotTag)
{
	...
	{
		reqSlotTag = GetFromFreeReqQ();
		XTime_GetTime(&st_read_log[reqSlotTag]);
		reqPoolPtr->reqPool[reqSlotTag].reqType = REQ_TYPE_NAND;
...
void ReqTransSliceToLowLevel()
{
	...
	while(sliceReqQ.headReq != REQ_SLOT_TAG_NONE)
	{
		...
		if(dataBufEntry != DATA_BUF_FAIL)
		{
			...
			// To measure value log hit ratio
			unsigned int cmdType = auto_cpl_info[reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag].mark;
//			if (cmdType == 0xFD || cmdType == 0xFE || cmdType == 0xFF) {
			if (cmdType >= 0xFD) {
				switch (cmdType) {
					case 0xFF: // Get
						stat[KV_GET].hit++;
		...
		else
		{
			...
			if(reqPoolPtr->reqPool[reqSlotTag].reqCode  == REQ_CODE_READ)
            		{
				DataReadFromNand(reqSlotTag);    // Default Code
            		}
...
void SelectLowLevelReqQ(unsigned int reqSlotTag)
{
	...
	if(bufDepCheckReport == BUF_DEPENDENCY_REPORT_PASS)
	{
		if(reqPoolPtr->reqPool[reqSlotTag].reqType  == REQ_TYPE_NVME_DMA)
		{
			...
			//xil_printf("\r\nComplete Issue & PutToNvmeDmaReqQ!!!\r\n");
		}
...
void IssueNvmeDmaReq(unsigned int reqSlotTag)
{
	...
	{
		// Piggyback <Key, Value Size>
		unsigned int cmdSlotTag = reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag;
		
		while(numOfNvmeBlock < reqPoolPtr->reqPool[reqSlotTag].nvmeDmaInfo.numOfNvmeBlock)
		{
			unsigned int autoCPL = (auto_cpl_info[cmdSlotTag].mark == 0) ? NVME_COMMAND_AUTO_COMPLETION_ON : NVME_COMMAND_AUTO_COMPLETION_OFF;
			// set_auto_tx_dma(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, dmaIndex, devAddr, NVME_COMMAND_AUTO_COMPLETION_ON);
			set_auto_tx_dma(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag, dmaIndex, devAddr, autoCPL);
			numOfNvmeBlock++;
			auto_cpl_info[cmdSlotTag].sentNvmeBlock++;
			dmaIndex++;
			devAddr += BYTES_PER_NVME_BLOCK;


			if (auto_cpl_info[cmdSlotTag].mark >= 0xFD &&  auto_cpl_info[cmdSlotTag].sentNvmeBlock >= auto_cpl_info[cmdSlotTag].numOfNvmeBlock) {
				//xil_printf("waiting for dma to be done\r\n");
				check_auto_tx_dma_done();
				//xil_printf("about to send complete\r\n");
				
				NVME_COMPLETION nvmeCPL;
				nvmeCPL.dword[0] = 0;
				nvmeCPL.specific = auto_cpl_info[cmdSlotTag].kv_length;
				set_auto_nvme_cpl(cmdSlotTag, nvmeCPL.specific, nvmeCPL.statusFieldWord);

				// re-initialize
				auto_cpl_info[cmdSlotTag].mark = 0;
				// auto_cpl_info[cmdSlotTag].sqid = 0;
				// auto_cpl_info[cmdSlotTag].cid = 0;
				auto_cpl_info[cmdSlotTag].numOfNvmeBlock = 0;
				auto_cpl_info[cmdSlotTag].sentNvmeBlock = 0;
				auto_cpl_info[cmdSlotTag].kv_length = 0;
			}
		}
