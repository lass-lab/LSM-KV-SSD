...
unsigned int GenerateDataBufAddr(unsigned int reqSlotTag)
{
	if(reqPoolPtr->reqPool[reqSlotTag].reqType == REQ_TYPE_NAND)
	{
		...
		else if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ADDR_NO_SPARE)
			return reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.addr;
	...
	switch(dieStateTablePtr->dieState[chNo][wayNo].dieState)
	{
		...
		case DIE_STATE_EXE:
			if(reqStatus == REQ_STATUS_DONE)
			{
				...
				{
					if(st_read_log[reqSlotTag] != 0) {
						XTime_GetTime(&ed_read_log[reqSlotTag]);
						stat[GetTypefromCmdSlotTag(reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag)].NAND_READ_LOG += ed_read_log[reqSlotTag] - st_read_log[reqSlotTag];
						ed_read_log[reqSlotTag] = st_read_log[reqSlotTag] = 0;
					}
					retryLimitTablePtr->retryLimit[chNo][wayNo] = RETRY_LIMIT;
